# Elderly Rejuvenation Health Plan

**Created:** 2026-04-08  
**Target:** Adults 60+ and their caregivers  
**Purpose:** Actionable plan for leveraging longevity research

---

## Executive Summary

This plan provides a **tiered approach** based on what's available NOW versus what's coming. The key insight: **AI is accelerating everything** — timelines may be faster than expected, so staying informed matters.

---

## Phase 1: Start Now (2026)

### What You Can Do TODAY

#### Tier A: Proven & Available (Do Now)

| Approach | What | Cost | Where to Get |
|----------|------|------|--------------|
| **NAD+ Boosters** | NMN or NR supplements | $20-80/mo | Online retailers, specialty shops |
| **Senolytics** | Fisetin or Quercetin | $20-40/mo | Online, health stores |
| **Vitamin D + Exercise** | Basic longevity stack | $10-20/mo | Anywhere |

**Recommendation:** Start NAD+ boosters (best evidence for 60+)

#### Lifestyle Interventions (Available Now, Free/Low Cost)

Research shows these have direct epigenetic effects:

| Intervention | Mechanism | Recommendation |
|--------------|-----------|----------------|
| **Exercise** | Releases molecules that go into cells and address epigenome, making cells more youthful | 150 min/week moderate, or 75 min vigorous |
| **Sleep** | Restorative sleep allows cellular repair; 8 hours optimal | Consistent schedule, cool environment |
| **Intermittent Fasting** | Triggers cellular repair (autophagy), affects epigenetic markers | 16:8 or 5:2 protocols |
| **Temperature Regulation** | Cool sleeping environment improves metabolic health | 65-68°F (18-20°C) bedroom |

**Note:** The video emphasizes that these lifestyle factors directly impact the epigenome — they aren't just "nice to haves" but actual interventions that can edge you toward more youthful cellular function while waiting for clinical therapies.

#### Tier B: Emerging (Consider Soon)

| Approach | What | When |
|----------|------|------|
| **Combination therapy** | NAD+ + Senolytics | 2026-2027 |
| **Blood work monitoring** | Track biomarkers | Now |

---

### Practical Steps for Age 60-80

1. **Start NAD+ precursors** (NMN or NR)
   - Dose: 250-500mg NMN daily (or 300mg NR)
   - Buy from reputable sources (check purity)
   
   **Recommended NMN Brands (2026):**
   - **HealthspanX Ultra Pure NMN** — 99%+ pure, 500mg/capsule, third-party tested, GMP certified — ~$40-60/mo
   - **GenuinePurity Liposomal NMN** — 96% pure, liposomal for better absorption — ~$50-70/mo
   - **ProHealth NMN Pro** — Good value powder form — ~$20-30/mo
   
   **What to check:**
   - Third-party lab testing (Certificate of Analysis available)
   - Purity 95%+ (ideally 98-99%)
   - GMP-certified manufacturing
   - No proprietary blends (single ingredient preferred)
   
2. **Add senolytic protocol** periodically
   - Fisetin: 20-100mg/kg (or 100mg daily)
   - Or Quercetin: 500mg 2x daily
   - Cycle: 3 days on, 11 days off (or as directed)
   
   **Recommended Senolytic Brands (2026):**
   - **Life Extension Senolytic Activator** — Quercetin + Fisetin combo, 36 caps, ~$25/mo
   - **ProSenolytic (ProHealth)** — High-dose fisetin — ~$30-40/mo
   - **NOW Foods Quercetin** — Budget option, 500mg — ~$15/mo

3. **Monitor biomarkers**
   - NAD+ levels
   - Inflammatory markers (IL-6, CRP)
   - Metabolic markers (glucose, HbA1c)

4. **Lifestyle foundation**
   - Exercise (especially strength + cardio)
   - Sleep (7-8 hours)
   - Diet ( Mediterranean style)

---

### Practical Steps for Age 80+

Same as above, plus:
- **Consult doctor** before starting any supplements
- **Start low** — lower doses, increase gradually
- **Monitor more closely** — work with healthcare provider
- **Focus on quality of life** — mobility, cognitive function

---

## Phase 2: Near-Term (2027-2029)

### What to Watch For

| Year | Expected Developments |
|------|---------------------|
| 2027 | More senolytic efficacy data |
| 2028 | First NAD+ booster FDA approval possible |
| 2028 | Therapeutic plasma exchange trials mature |
| 2029 | Combination therapy protocols refined |

### Action Items

- [ ] **Track trials** — Sign up for trial alerts (clinicaltrials.gov)
- [ ] **Consider joining trials** — If eligible
- [ ] **Update supplements** — As new evidence emerges
- [ ] **Re-evaluate annually** — New options may appear

---

## Phase 3: Future Therapies (2030+)

### What's Coming

| Therapy | When | Cost | Who Should Watch |
|---------|------|------|------------------|
| Epigenetic reprogramming | 2031-2035 | $10k+ | Younger elderly (60-70) |
| Klotho gene therapy | 2028-2030 | TBD | Those with cognitive concerns |
| Advanced telomere therapy | 2030+ | TBD | Those interested in early adoption |
| Comprehensive combos | 2032+ | TBD | Everyone |

### Preparation Steps

1. **Stay informed** — Monthly research updates
2. **Build relationships** — With longevity-focused clinics
3. **Financial planning** — Set aside funds for future therapies
4. **Health baseline** — Get comprehensive testing now

---

## AI Acceleration Factor

**Important:** AI is dramatically accelerating longevity research. Timelines may be faster than projected.

| Original Estimate | AI-Accelerated可能 |
|-------------------|-------------------|
| 2030+ for reprogramming | 2028-2030 |
| 2028 for NAD+ Rx | 2027 |
| 2030+ for combination | 2028-2029 |

### What This Means

- **Stay flexible** — Plans may need to adjust quickly
- **Monitor closely** — Monthly updates recommended
- **Be ready** — When new options emerge, act fast
- **Don't dismiss** — Things that seem far may come sooner

---

## Complete Action Plan

### For Age 60-70

```
Now (2026):
├─ Start NAD+ boosters ($20-80/mo)
├─ Consider senolytic protocol
├─ Establish baseline health markers
└─ Exercise + sleep + diet

2027-2029:
├─ Watch for FDA approvals
├─ Consider combination therapy
├─ Explore clinical trials
└─ Update based on new evidence

2030+:
├─ Evaluate reprogramming (if ready)
├─ Consider gene therapy options
└─ Combination approaches
```

### For Age 70-80

```
Now (2026):
├─ Start NAD+ boosters (consult doctor)
├─ Start senolytics (lower dose)
├─ Comprehensive baseline testing
└─ Focus on mobility + cognition

2027-2029:
├─ NAD+ booster FDA approval likely
├─ Senolytic protocols refined
└─ Monitor for new options

2030+:
├─ Evaluate gene therapy (if safe)
└─ Focus on quality of life
```

### For Age 80+

```
Now (2026):
├─ Consult healthcare provider FIRST
├─ Start low-dose NAD+ if approved
├─ Focus on existing health conditions
└─ Prioritize quality of life

2027-2029:
├─ Watch for senior-specific trials
└─ Maintain current protocols

2030+:
├─ Evaluate if therapies appropriate
└─ Individual decision based on health
```

---

## Budget Planning

| Tier | Monthly Cost | One-Time Cost |
|------|--------------|---------------|
| **Basic (supplements)** | $30-100 | $0 |
| **Plus (monitoring)** | +$50-100 | $200-500/yr testing |
| **Premium (future)** | +$50 | $10k+ reserved |

**Recommendation:** Start with Basic, add Plus when able, reserve funds for Premium.

---

## Monitoring Checklist

### Monthly
- [ ] Take supplements consistently
- [ ] Exercise 150 minutes
- [ ] Sleep 7-8 hours

### Quarterly
- [ ] Review new research
- [ ] Check for new trials
- [ ] Update health metrics

### Annually
- [ ] Comprehensive blood work
- [ ] Re-evaluate plan
- [ ] Adjust based on new evidence

---

## Key Resources

| Resource | URL | Purpose |
|----------|-----|---------|
| ClinicalTrials.gov | clinicaltrials.gov | Find trials |
| Lifespan.io | lifespan.io | Research updates |
| Reddit r/longevity | reddit.com/r/longevity | Community discussion |
| NAD+ Lookup | nadpool.com | Find testing |

---

## Emergency Contacts

**When to contact doctor:**
- New symptoms after starting supplements
- Interactions with existing medications
- Any concerns about changes

---

## Disclaimer

⚠️ **Important:** This is not medical advice. Always consult with healthcare providers before starting any new supplements or treatments. Individual circumstances vary.

---

*Health plan created: 2026-04-08*
*Review monthly for updates*
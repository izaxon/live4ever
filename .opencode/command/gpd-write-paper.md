...
For detailed mode adaptation specifications (bibliographer search breadth, referee strictness, paper-writer style by mode), see `./.opencode/get-physics-done/references/publication/publication-pipeline-modes.md`.
...
- Define all custom macros in a preamble block (see `./.opencode/get-physics-done/templates/latex-preamble.md` for standard packages, project-specific macros, equation labeling conventions, and SymPy-to-LaTeX integration)
...
This emits `paper/main.tex`, writes the artifact manifest, and keeps the manuscript scaffold aligned with the tested `gpd.mcp.paper` package. If no JSON spec exists yet, create `paper/PAPER-CONFIG.json` first using `./.opencode/get-physics-done/templates/paper/paper-config-schema.md` as the schema source of truth, and then run `gpd paper-build` before proceeding. The compilation checks in `draft_sections` require `main.tex` to exist.

When authoring `paper/PAPER-CONFIG.json`:

- use the exact top-level fields from `./.opencode/get-physics-done/templates/paper/paper-config-schema.md`
...
**Supplemental material:** If the paper requires supplemental material (common for PRL and other letter-format journals), use `./.opencode/get-physics-done/templates/paper/supplemental-material.md` for the standard structure (extended derivations, computational details, additional figures, data tables, code availability).

**Experimental comparison:** If the paper compares theoretical predictions with experimental or observational data, use `./.opencode/get-physics-done/templates/paper/experimental-comparison.md` for the systematic comparison structure (data source metadata, unit conversion checklist, pull analysis, chi-squared statistics, discrepancy classification with root cause hierarchy).
</step>
...
```
task(
  prompt="First, read ./.opencode/agents/gpd-paper-writer.md for your role and instructions.\n\n" + section_prompt,
...
task(
  ...
  model="{biblio_model}",
  readonly=false,
  prompt="First, read ./.opencode/agents/gpd-bibliographer.md for your role and instructions.
...
- `./.opencode/get-physics-done/templates/paper/reproducibility-manifest.md`
...
**7. Run paper quality scoring** (see `./.opencode/get-physics-done/references/publication/paper-quality-scoring.md`):
...
<process>
**Follow the write-paper workflow** from `@./.opencode/get-physics-done/workflows/write-paper.md`.
...
7. **Generate files** — Create `paper/PAPER-CONFIG.json` using `@./.opencode/get-physics-done/templates/paper/paper-config-schema.md`, then materialize the canonical manuscript scaffold with `gpd paper-build` (emits `paper/main.tex`, bibliography artifacts, and `paper/ARTIFACT-MANIFEST.json`)

...
```bash
GPD_INSTALL_DIR="./.opencode/get-physics-done"
GPD_CONFIG_DIR="./.opencode"
GPD_GLOBAL_CONFIG_DIR="/home/johan/.config/opencode"
GPD_RUNTIME_FLAG="--opencode"
INSTALL_SCOPE="--local"
...
> **Platform note:** If `question` is not available, present the choices in plain text and wait for the user's freeform response.
...
```bash
rm -f \
  "./.opencode/cache/gpd-update-check.json" \
  "/home/johan/.config/opencode/cache/gpd-update-check.json" \
...
<process>
**Follow the update workflow** from `@./.opencode/get-physics-done/workflows/update.md`.

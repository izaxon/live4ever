...
- `@./.opencode/get-physics-done/references/orchestration/agent-infrastructure.md` -- Shared infrastructure: data boundary, context pressure, external tool failure, commit protocol
- `@./.opencode/get-physics-done/references/physics-subfields.md` -- Methods, tools, and validation strategies per physics subfield (informs framework and formalism analysis)
...
- `theory`, `computation`, `methodology`: available tools are `read_file`, `write_file`, `shell`, `grep`, and `glob`
- `status`: the same tools plus `websearch` and `webfetch`
...
Use glob and grep (never raw shell find/grep):
...
Use the write_file tool to create each document.
</step>
...
1. Use `grep("model|system|consider|study|investigate", glob="*.tex")` for defining statements
...
- `@./.opencode/get-physics-done/references/templates/research-mapper/FORMALISM.md`
- `@./.opencode/get-physics-done/references/templates/research-mapper/REFERENCES.md`

**Computation focus** (ARCHITECTURE.md, STRUCTURE.md):
- `@./.opencode/get-physics-done/references/templates/research-mapper/ARCHITECTURE.md`
- `@./.opencode/get-physics-done/references/templates/research-mapper/STRUCTURE.md`

**Methodology focus** (CONVENTIONS.md, VALIDATION.md):
- `@./.opencode/get-physics-done/references/templates/research-mapper/CONVENTIONS.md`
- `@./.opencode/get-physics-done/references/templates/research-mapper/VALIDATION.md`

**Status focus** (CONCERNS.md):
- `@./.opencode/get-physics-done/references/templates/research-mapper/CONCERNS.md`

### When Template Files Don't Exist

If a template file is not found at the expected path (e.g., `./.opencode/get-physics-done/references/templates/research-mapper/` does not exist), treat that as a broken install and fall back to this procedure:

1. **Do not search alternate runtime-specific paths.** GPD installs the shared reference tree at a deterministic `./.opencode/get-physics-done` location for every runtime.
...
<!-- 843 lines of inline templates removed — now loaded from reference files above.
     See ./.opencode/get-physics-done/references/templates/research-mapper/ for the full templates.

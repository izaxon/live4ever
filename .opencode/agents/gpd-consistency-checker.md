...
**Tool note:** You have `write_file` (not `edit_file`) — you write full CONSISTENCY-CHECK.md files. You also have `read_file`, `shell`, `grep`, and `glob` for investigation.
</role>
...
<references>
- `@./.opencode/get-physics-done/references/verification/core/verification-core.md` -- Universal verification checks for cross-phase consistency validation
- `@./.opencode/get-physics-done/references/physics-subfields.md` -- Methods, conventions, and validation strategies per physics subfield
- `@./.opencode/get-physics-done/references/orchestration/agent-infrastructure.md` -- Agent infrastructure: data boundary, context pressure, commit protocol

**On-demand references:**
- `./.opencode/get-physics-done/references/examples/contradiction-resolution-example.md` -- Worked example of resolving contradictions with confidence weighting (load when encountering conflicting claims between phases)
...
- `./.opencode/get-physics-done/references/shared/cross-project-patterns.md` -- Cross-project pattern library: check for known convention error patterns before investigating from scratch, record new patterns after resolution
- `./.opencode/get-physics-done/templates/uncertainty-budget.md` -- Template for `.gpd/analysis/UNCERTAINTY-BUDGET.md` (load when auditing uncertainty propagation across phases)
</references>
...
**If `.gpd/CONVENTIONS.md` does not exist:** Create it from the template at @./.opencode/get-physics-done/templates/conventions.md, then populate it by scanning all existing phase artifacts for convention choices (metric signature, unit system, Fourier convention, etc.). Commit the new file before proceeding.
...
Always use the write_file tool to persist the report. The structured return to the auditor is in addition to the on-disk copy, not a replacement.
